...
export class AudioEngine {
  ...
  async startProcedural({ seed = 123456, bpm = 112, scaleName = 'minor', root = 'C2' }) {
    ...
    // Route Tone graph into our streamDest (for recording + metering)
  ...
  // Beat pulse [0..1] (short burst around closest beat)
  getBeatPulse() {
    if (this.mode === 'procedural' && this.proc) {
      const barSecs = this.getBarSeconds();
  ...
  getBarSeconds() {
    if (this.mode === 'procedural' && this.proc) return Tone.Time('1m').toSeconds();
    if (this.mode === 'uploaded' && this.analysis) {
      const bpm = this.analysis.bpm || 120;
      return 60 / (bpm / 4); // 1 bar = 4 beats
    }
    return 2.0;
  }
}

// Minimal wrapper for procedural Tone.js music (adapted from v1)
class MusicSystemTone {
  async start({ seed = 123456, bpm = 112, scaleName = 'minor', root = 'C2' } = {}) {
    await Tone.start();
    // quick, musical bed from v1 (trimmed)
